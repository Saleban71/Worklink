...
```
┌────────────────────────┐
│   Flutter app (app/)   │  Android first, iOS Phase 2
│  screens → services →  │
└───────────┬────────────┘
            │ supabase_flutter (Auth, PostgREST, Realtime, Storage)
┌───────────▼────────────┐
│       Supabase         │
│  Postgres + PostGIS    │  ← schema, RLS, matching RPC, escrow triggers
│  Edge Functions (Deno) │  ← generate-profile (Claude API)
│  Auth (phone OTP)      │
└───────────┬────────────┘
            │ webhooks (production)
┌───────────▼────────────┐
...
### 3. Mobile money (production)

`PaymentService.fundEscrow` currently writes the escrow row directly — the **sandbox/dev path**. For production:

1. Create an edge function `initiate-collection` that calls the MTN MoMo Collections API (`requesttopay`) / Airtel Money equivalent.
2. Create a webhook edge function that receives the provider callback and sets `escrow_transactions.status = 'funded'` using the service role.
3. Do the same for disbursement on release (`payouts` table is already in place).
4. Remove the client-side status writes.

MoMo sandbox: https://momodeveloper.mtn.com
